...
hideSelectTags = [];
var showDatePickAlert = false;  // Call a function on picking a date.
var inputFieldId;    // id of the input element in which picked date will get set.
...
function showCalender(elPos, tgtEl, showAlert)
{
//      alert(elPos +" , "+ tgtEl);
    showDatePickAlert = showAlert;
    inputFieldId = tgtEl;
    //   alert("hhhh=---"+test);
    targetEl = false;
...
function closeCalender()
{
    ...
    {
        hideSelectTags[i].style.display = 'inline-block';
    }
...
function pickDate(year, month, day)
{
    ...
    else
    {
        ...
        targetEl.value = pickedDate;
        closeCalender();
    }
    //alert("hi---"+test)
    if(showDatePickAlert){
        datePicked(inputFieldId, pickedDate);    // Call a function on picking a date.
    }
}
